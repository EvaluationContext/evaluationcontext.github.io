...
In SQLBI's [DAX Naming Conventions](https://docs.sqlbi.com/dax-style/dax-naming-conventions#comments), it suggests using the [JSDoc standard](https://jsdoc.app/) for DAX UDF function comments. Specifically `@param` and `@return`.
...
??? info "Supported Tags"
    ...
    !!! info "@tag" 
        ...
        My script supports the following tag, but it is suggested to limit the use of tags to `@param` and `@return`. This keeps the functions comments shown in Power BI IntelliSense limited to those required for people to understand the interface to your function. This also reduces the size of comments in the `functions.tmdl` which is important since there are limitations on size of this file that can be submitted to DAX Lib. Examples and other extra content can be included in a site page by using markdown files in `extended_content` which will be merge with JSDocs comments from `functions.tmdl`.
...
1. Content that appears **before** the tabbed sections (Syntax/Definition)
2. Additional tabs that appear **alongside** the standard Syntax/Definition tabs (i.e. examples)
